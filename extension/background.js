...
async function gettext(info) {
    ...
    extWindow = window.open('./index.html', 'extWindow');
    console.log('Text ' + info.selectionText + ' was clicked.');
    extWindow.getElementById('text').innerHTML = info.selectionText;
    var simplification = await simplifyText(info.selectionText).then((response) => {return response});
    console.log(info.selectionText + ' -> ' + simplification);
